...
angular.module('sortApp', [])
  ...
  $scope.fileName = $scope.audioFileNames[0][0][0];
...
$scope.setSetting = function(indexTable, row) {
  ...
  $scope.selectedTable = indexTable;
  $scope.fileName = $scope.audioFileNames[indexTable][$scope.selectedSetting][$scope.fileId];
  $scope.playAudio();
}
...
$scope.fileId = Math.floor(Math.random()*$scope.audioFileNames[$scope.selectedTable][$scope.selectedSetting].length);
    ...
    $scope.fileName = $scope.audioFileNames[$scope.selectedTable][$scope.selectedSetting][$scope.fileId];
  }
