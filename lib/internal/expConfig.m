...
if ~isempty(expGetMachineName(config, config.host)) && strcmp(config.localHostName, expGetMachineName(config, config.host)) && ~strcmp(config.experimentPath, fileparts(which(config.experimentName)))
