...
if isempty(regexp(config.emailAddress, '[a-z_]+@[a-z]+\.[a-z]+', 'match'))
    ...
    if ~atEnd
                title = ['[expCode] ' config.projectName ' id ' num2str(config.runId) ' is launched on ' config.hostName];
    else
        title = ['[expCode] ' config.projectName ' id ' num2str(config.runId) ' is over on ' config.hostName];
    end
