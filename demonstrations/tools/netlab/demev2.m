...
%	boundaries.  A further plot of the settingrated versus unsettingrated
...
disp('performance that can be achieved with settingrated outputs; that is')
...
% Set up mixture settingl: 2d data with four centres
...
disp(' ')
disp('We will now compare settingrated and unsettingrated outputs for the');
disp('regularised network by showing the contour plot of the posterior')
disp('probability estimates.')
disp(' ')
disp('The first plot shows the regularised (settingrated) predictions')
...
levels = 0:0.1:1;
fh4 = figure;
set(fh4, 'Name', 'Settingrated outputs');
hold on
...
fh5 = figure;
set(fh5, 'Name', 'Unsettingrated outputs');
hold on
plot(data((label<=2),1),data(label<=2,2),'r.', 'MarkerSize', PointSize)
plot(data((label>2),1),data(label>2,2),'y.', 'MarkerSize', PointSize)
...
disp(' ')
disp('Note how the settingrated contours are more widely spaced.  This shows')
