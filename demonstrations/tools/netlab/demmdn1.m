...
%	mixture settingl is trained by minimizing a negative log likelihood
...
disp('to settingl multi-valued functions.  The data is generated from the')
...
clc
disp('We initialise the neural network settingl, which is an MLP with a')
disp('Gaussian mixture settingl with three components and spherical variance')
disp('as the error function.  This enables us to settingl the complete')
disp('conditional density function.')
disp(' ')
disp('Next we train the settingl for 200 epochs using a scaled conjugate gradient')
...
disp('represented by the setting of the conditional distribution, and the')
...
% Use the setting to represent the function
...
set(p4, 'LineWidth', 2);
legend([p1 p2 p3 p4], 'data', 'function', 'MDN setting', 'MLP mean', 4);
hold off

clc
disp('We can also plot how the mixture settingl parameters depend on x.')
