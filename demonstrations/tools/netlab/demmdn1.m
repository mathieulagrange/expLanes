...
%	mixture designl is trained by minimizing a negative log likelihood
...
disp('to designl multi-valued functions.  The data is generated from the')
...
clc
disp('We initialise the neural network designl, which is an MLP with a')
disp('Gaussian mixture designl with three components and spherical variance')
disp('as the error function.  This enables us to designl the complete')
disp('conditional density function.')
disp(' ')
disp('Next we train the designl for 200 epochs using a scaled conjugate gradient')
...
disp('represented by the design of the conditional distribution, and the')
...
% Use the design to represent the function
...
set(p4, 'LineWidth', 2);
legend([p1 p2 p3 p4], 'data', 'function', 'MDN design', 'MLP mean', 4);
hold off

clc
disp('We can also plot how the mixture designl parameters depend on x.')
